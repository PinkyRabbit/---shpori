...
```
## Ветка Hotfix
Ветка _hotfix_ происходит из ветки _master_ и сливается в неё после завершения хотфикса. По негласному соглашению называется `hotfix/*`. Эта ветка создаётся после того, как в релизе обнаружен критический баг, требующий срочного исправления.

Причина создания такой ветки аналогична с созданием ветки релиза. Нам нужно исправить баг быстро, а не путать работу людям в ветке development. Как только баг исправлен, ветка сливается в _master_ и _develop_ и на этом перестаёт существовать. Обычно, после этого на мастера клеят новый тег, означающий версию с исправлением. Например, v1.1.1-fix
### Пример:
Создаём хотфикс
```
git checkout master
...
```
И делаем все исправления. После коммитим их:
```
git status
git add .
git commit -am "Your message"
```
Публикуем наши исправления
```
...
```
## Уточнение
Где-то команды могут идти в ненужном порядке, но здесь важно уловить суть. Суть, как работает `Gitflow` и почему люди испольуют его в своих проектах. `Gitflow` - это расширение гита и устанавливается отдельно от него. Можно обойтись и без него, но... но с ним - привычнее.
